...
class WeatherMapper extends Mapper<Text, Text, MyKey, DoubleWritable> {
    ...
    protected void map(Text key, Text value, Mapper<Text, Text, MyKey, DoubleWritable>.Context context)
        ...
        System.out.println("filename: "+inputSplit.getPath().getName()+"-------------------------------------");
        int id = 0;
        if(filename.equals("London2013.csv")){
            id = 1;
        }else if(filename.equals("Mumbai2013.csv")){
            id = 2;
        }else if(filename.equals("NewYork2013.csv")){
            id = 3;
        }else if(filename.equals("SFO2013.csv")){
            id = 4;
        }else{
            id = 5;
        }
...
//        System.out.println("month: " + month + "  ------------------------------------------------------");
        ...
        context.write(mykey, new DoubleWritable(temperature));
...
class WeatherReduce extends Reducer<MyKey, DoubleWritable, Text, DoubleWritable> {
    NullWritable nullWritable = NullWritable.get();
    @Override
    protected void reduce(MyKey arg0, Iterable<DoubleWritable> arg1,
                          Reducer<MyKey, DoubleWritable, Text, DoubleWritable>.Context arg2)
            throws IOException, InterruptedException {
